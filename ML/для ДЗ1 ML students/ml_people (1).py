...
def classifyStudent(new_student, model_file, new_columns=['a', 'b', 'c', 'd', 'e', 'f'], categories=['A', 'B', 'C']):
    '''
    ...
    predicted_category = int(model.predict(new_data)[0])
...
def preprocess_dataset(
    ...
    if (need_to_encode_y):
        ...
        new_result.to_csv('result_labels.csv', index=False)
        y = temp       
        print(y)
